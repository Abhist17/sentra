...
// ── Per-wallet live metrics ──────────────────────────────────────
const walletMetrics: Map<
  string,
...
export function registerRoutes(app: Express) {
  ...
  app.post("/wallet/add", (req, res) => {
    try {
      ...
      res.json({ success: true, message: `Now monitoring ${entry.label}`, wallet: entry });
    } catch (err) {
      res.status(400).json({
        success: false,
  ...
  app.get("/snapshots", async (req, res) => {
    try {
      ...
      if (!walletParam) return res.status(400).json({ error: "wallet address required" });

      const provider  = createProvider();
      const program   = getProgram(provider);
      const snapshots = await fetchUserSnapshots(program, new PublicKey(walletParam));
  ...
  /* =============================
     POST /test/alert
     Manually trigger a Telegram alert
     Great for demos and testing
     curl -X POST http://localhost:4000/test/alert
  ============================= */
  app.post("/test/alert", async (req, res) => {
    try {
      // Use latest real metrics if available, otherwise use defaults
      ...
      const portfolioValue = metrics?.portfolio
        ? `$${metrics.portfolio.toLocaleString("en-US", { maximumFractionDigits: 2 })}`
        : "$1,326,354,419";
      ...
      await sendTelegramAlert(
        ...
        `📉 SOL: $${latestPortfolio > 0 ? (latestPortfolio / 1e6).toFixed(2) : "89.40"}\n\n` +
        `⚡ Powered by Sentra`
      );
  ...
  /* =============================
     POST /test/shock
     Simulate a market shock alert
     curl -X POST http://localhost:4000/test/shock
  ============================= */
  app.post("/test/shock", async (req, res) => {
    try {
      await sendTelegramAlert(
      ...
      res.json({ success: true, message: "Market shock alert sent to Telegram ✅" });
    } catch (err) {
      res.status(500).json({
        success: false,
        error: err instanceof Error ? err.message : "Failed to send alert",
